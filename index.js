...
function find(array, callback) {
  for (let element of array) {
    if (callback(element)) {
      return element; // Return the first element that satisfies the callback
    }
  }
...
function filter(array, callback) {
  ...
  for (let element of array) {
    if (callback(element)) {
      result.push(element); // Push elements that satisfy the callback to the result array
    }
  }
  return result
}
...
function forEach(array, callback) {
  for (let i = 0; i < array.length; i++) {
    callback(array[i], i, array); // Pass element, index, and array to the callback
  }
}
